...
void MemoryTraceManager::OnDmaRead(u32 addr, u32 pc)
{
	ProcessAccess(addr, 16, pc, false, true);
}
...
std::vector<TraceEntry> MemoryTraceManager::GetHookTraceStats(u32 hookId) const
{
	std::unique_lock lock(m_mutex);
...
void MemoryTraceManager::ProcessAccess(u32 addr, u32 size, u32 pc, bool isWrite, bool isDma)
{
	std::unique_lock lock(m_mutex);
	...
	for (auto& [id, hook] : m_hooks)
	{
		...
		for (const auto& range : hook.ranges)
		{
			...
			// Helper to record an access in first-seen order
			auto recordAccess = [&hook, isDma](u32 accessAddr, u32 rangeStart, u32 accessPC) {
				TraceKey key;
				key.offset = accessAddr - rangeStart;
				key.stack = CaptureStackTrace(accessPC);
				auto indexIt = hook.statsIndex.find(key);
				if (indexIt != hook.statsIndex.end())
				{
					hook.stats[indexIt->second].count++;
				}
				else
				{
					u32 idx = static_cast<u32>(hook.stats.size());
					hook.statsIndex[key] = idx;
					TraceEntry entry;
					entry.key = key;
					entry.count = 1;
					entry.isDma = isDma;
					hook.stats.push_back(entry);
				}
			};

			if (isDma)
			{
				if (hook.flags & MEMTRACE_TRACK_DMA)
				{
					recordAccess(addr, range.start, pc);
				}
			}
			else if (isWrite)
			{
