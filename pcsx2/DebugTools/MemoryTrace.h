...
// A single trace stat entry preserving insertion order
struct TraceEntry
{
	TraceKey key;
	u32 count = 0;
...
struct HookTrace
{
	...
	std::vector<TraceEntry> stats;       // entries in first-seen order
	std::map<TraceKey, u32> statsIndex;  // key -> index into stats vector
	u32 firstWritePC = 0;
...
public:
	...
	void OnDmaRead(u32 addr, u32 pc);

	// Query
	bool HasActiveTraces() const;

	// Get trace stats for UI display (returns copy for thread safety, in first-seen order)
	std::vector<TraceEntry> GetHookTraceStats(u32 hookId) const;
...
private:
	...
	void ProcessAccess(u32 addr, u32 size, u32 pc, bool isWrite, bool isDma = false);
